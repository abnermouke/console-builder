        ...
        ($builder = (new \Abnermouke\ConsoleBuilder\Builders\Form\ConsoleFormBuilder(\App\Builders\Abnermouke\Console\ConsoleBuilderBasicTheme::FORM_WITH_KM8)))
            ...
            ->setItems(function () {
                return ($formItemBuilder = (new \Abnermouke\ConsoleBuilder\Builders\Form\Tools\FormItemsBuilder()))
                    ...
                    ->addItem($formItemBuilder->buildSwitch('QINIU_SYNC_AUTO', '文件资源自动上传七牛云')->allow_text('允许')->on(\App\Model\Abnermouke\Console\Configs::SWITCH_ON, ['QINIU_PARAMS__domain', 'QINIU_PARAMS__access_key', 'QINIU_PARAMS__access_secret', 'QINIU_PARAMS__bucket', 'QINIU_PARAMS__visibility'])->off(\App\Model\Abnermouke\Console\Configs::SWITCH_OFF))
                    ->addItem($formItemBuilder->buildInput('WECHAT_OFFICE_ACCOUNT_PARAMS__app_id', '微信公众号APPID')->clipboard()->required())
                    ->addItem($formItemBuilder->buildInput('WECHAT_OFFICE_ACCOUNT_PARAMS__secret', '微信公众号APP密钥')->clipboard()->required())
                    ->addItem($formItemBuilder->buildInput('WECHAT_OFFICE_ACCOUNT_PARAMS__token', '微信公众号通讯TOKEN')->clipboard()->required())
                    ->addItem($formItemBuilder->buildInput('WECHAT_OFFICE_ACCOUNT_PARAMS__aes_key', '微信公众号AES KEY')->clipboard()->required())
                    ->addItem($formItemBuilder->buildInput('QINIU_PARAMS__domain', '七牛访问域名', 'url')->required())
                    ->addItem($formItemBuilder->buildInput('QINIU_PARAMS__access_key', '七牛云 AccessKey')->clipboard()->required())
                    ->addItem($formItemBuilder->buildInput('QINIU_PARAMS__access_secret', '七牛云 AccessKey Secret')->clipboard()->required())
                    ->addItem($formItemBuilder->buildInput('QINIU_PARAMS__bucket', '储存空间bucket')->clipboard()->default_value('public')->required())
                    ->addItem($formItemBuilder->buildRadio('QINIU_PARAMS__visibility', '储存空间性质')->value_type(\Abnermouke\ConsoleBuilder\Builders\Form\Items\BasicItemBuilder::VALUE_TYPE_OF_STRING)->options_with_descriptions(['public' => '公有云', 'private' => '私有云'], ['public' => '可通过文件对象的 URL 直接访问。', 'private' => '文件对象的访问则必须获得拥有者的授权才能访问。']))
                    ->addItem($formItemBuilder->buildEditor('PRIVACY_POLICY', '隐私政策', \Abnermouke\ConsoleBuilder\Builders\Form\Items\EditorItemBuilder::CK_EDITOR))
                    ->addItem($formItemBuilder->buildEditor('USER_AGREEMENT', '用户服务协议', \Abnermouke\ConsoleBuilder\Builders\Form\Items\EditorItemBuilder::CK_EDITOR))
            ...
            ->addStructure($builder->buildStructure()->title('七牛云服务配置')->addFiled('QINIU_SYNC_AUTO', 12)->addFiled('QINIU_PARAMS__domain', 6)->addFiled('QINIU_PARAMS__access_key', 6)->addFiled('QINIU_PARAMS__access_secret', 6)->addFiled('QINIU_PARAMS__bucket', 6)->addFiled('QINIU_PARAMS__visibility', 12))

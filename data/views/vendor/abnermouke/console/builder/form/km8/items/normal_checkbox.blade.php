    ...
    @foreach(data_get($extras, 'options', []) as $value => $guard_name)
        <div class="d-flex fv-row my-5">
            <div class="form-check form-check-custom form-check-solid">
                <input class="form-check-input me-3  acbf_{{ $sign }}_item_{{ $field }}_checkbox_item" type="checkbox" id="acbf_{{ $sign }}_item_{{ $field }}_{{ $value }}" name="{{ $field }}" {{ $readonly ? 'readonly' : '' }} {{ $disabled ? 'disabled' : '' }} value="{{ $value }}" @if(in_array($value, object_2_array($default_value))) checked="checked" @endif>

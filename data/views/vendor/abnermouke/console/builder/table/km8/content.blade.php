                            ...
                            <tr class="acbt_{{ $sign }}_table_tbody_tr" id="acbt_{{ $sign }}_table_tbody_tr_{{ $k }}">
                                ...
                                @if($actions)
                                    <td class="acbt_{{ $sign }}_table_tbody_td_actions text-end export_ignore">
                                        ...
                                        @else
                                            <a href="javascript:;" class="btn acbt_{{ $sign }}_table_action_item btn-sm btn-light-{{ $actions[0]['theme'] }}"  data-ajax-after="{{ $actions[0]['after_ajax'] }}" data-method="{{ $actions[0]['method'] }}" data-type="{{ $actions[0]['type'] }}" data-query-url="{{ get_acbt_link($actions[0]['redirect_uri'], $list) }}" data-target-redirect="{{ (int)$actions[0]['redirect_target'] }}" data-confirm-tip="{{ $actions[0]['confirm_tip'] }}" data-param-fields="{{ json_encode(\Illuminate\Support\Arr::only($list, $actions[0]['param_fields'])) }}" data-extras="{{ json_encode($actions[0]['extras']) }}">{!! $actions[0]['icon'] ? '<i class="'.$actions[0]['icon'].'"></i>' : '' !!}{{ $actions[0]['guard_name'] }}</a>
                                        @endif
                                    </td>
                                @endif
                            </tr>
                    ...
                    @else
                        ...
                        <tr>
                            <td colspan="12">
                                ...
                                <div class="d-flex flex-row-auto bgi-no-repeat bgi-position-x-center bgi-size-contain bgi-position-y-bottom min-h-150px min-h-lg-350px md-10" style="background-image: url({{ proxy_assets('themes/km8/media/illustrations/sketchy-1/18'.($console_configs['CONSOLE_DEFAULT_THEME'] == 'auto' ? (((int)date('H') >= 19 || (int)date('H') <= 6) ? '-dark' : '') : ($console_configs['CONSOLE_DEFAULT_THEME'] == 'dark' ? '-dark' : '')).'.png', 'abnermouke') }})"></div>
                            </td>
                        </tr>
                    @endif
                </tbody>

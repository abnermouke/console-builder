...
class TagsItemBuilder extends BasicItemBuilder
{
    ...
    {
        //设置基础信息
        $this->setParam('type', 'tags')->guard_name($guard_name)->field($field)->placeholder('输入（标签/关键词）后回车确认以生成（标签/关键词）信息')->description('输入（标签/关键词）后回车确认以生成（标签/关键词）信息')->tip('标签/关键词将进行自动过滤，如存在违禁词将在提交后自动删除');
    }
